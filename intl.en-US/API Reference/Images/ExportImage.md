...
-   Supports custom mirrors that export information that includes disk snapshots in mirrors, the number of data disks cannot exceed 4, and the maximum capacity of a single data disk cannot exceed 500 GiB.
...
|RegionId|String|Yes|The RegionId of the custom image. For more information, query the latest region ID list by calling [DescribeRegions](reseller.en-US/API Reference/Regions/DescribeRegions.md#).|
...
|MissingParameter|An input parameter RegionId that is required for processing the request is not supplied.|400|The `RegionId` parameter is required.|
|MissingParameter|An input parameter ImageId that is required for processing the request is not supplied.|400|The `ImageId` parameter is required.|
|MissingParameter|An input parameter OssBucket that is required for processing the request is not supplied.|400|The `OssBucket` parameter is required.|
