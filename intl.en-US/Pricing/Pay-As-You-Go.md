...
-   **Resource configuration change**: You can [change the instance types, including CPU configuration and memory capacity, after you create an instance](https://www.alibabacloud.com/help/doc-detail/60051.htm).
-   **Billing method change**: Instances, system disks, and data disks support switching from **Pay-As-You-Go** to **Subscription**. For more information about the procedure, see [Switch from Pay-As-You-Go to Subscription](https://www.alibabacloud.com/help/doc-detail/49884.htm).
...
A Pay-As-You-Go resource is billed by the second after is it created and stops billing after it is released. However, you can enable the [no fees for stopped instances feature](intl.en-US/Pricing/No fees for stopped instances (VPC-Connected).md#) for a VPC instance. After the feature is enabled, a VPC instance is billed by second after it is created, stops billing when it is in the **Stopped** status. And it starts billing again when it starts again. Except for instances, **this feature** does not apply to other ECS resources.
...
\*\*The **Expired** status means the instance stops working because of overdue payment. The status of the instance is **Expired**. During the period of out of service, no fees are incurred.
...
-   VPC: You can enable the **No fees** for stopped instances feature. When this feature is enabled, an instance stops billing when it is in the **Stopped** status and starts billing again when it is started. **This feature** does not affect the billing of other resources.
