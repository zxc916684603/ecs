...
You must maintain sufficient balance to your linked credit card or 

PayPal account to complete the payment or preauthorization. For more information, see Subscription and Pay-As-You-Go.
...
-   `PrivateIpAddress` depends on `VSwitchId` and `PrivateIpAddress`  cannot be specified separately. 
...
-   When you create an instance, `InternetChargeType` and `InternetMaxBandwidthOut` determine the bandwidth costs. `The value of InternetMaxBandwidthIn` is not related to billing, and the inbound data traffic for the instance is free.

-   The relationship between `InternetChargeType` and `InternetMaxBandwidthOut` is shown in the following table.

    |**Parameters**|`InternetChargeType`|`InternetMaxBandwidthOut`|
    |:-------------|:-------------------|:------------------------|
    |**Value**|`PayByTraffic`|PayByTraffic. The fees are collected according to actual traffic usage.|
...
-   ECS allocates a system disk of an appropriate size for the instance based on the specified ImageId. The available disk categories for system disk are basic cloud disk \(`cloud`\), ultra cloud disk \(`cloud_efficiency`\), SSD cloud disk \(`cloud_ssd`\), and ephemeral SSD disk \(`ephemeral_ssd`\).
...
-   ECS does not allow users to separately create or add an ephemeral SSD. Therefore, the ephemeral SSD must be specified during instance creation. For disk category and size restrictions for different instance type, see [Instance type families](../../../../intl.en-US/Product Introduction/Instance type families.md#).


**User-defined data**

User-defined data is a function to customize the startup behaviors of an ECS instance and to pass data into an ECS instance.  An ECS instance must meet all the requirements to be able to input UserData. UserData is encoded in Base64 format. UserData transmission is not encrypted. We recommend that you do not input confidential information, such as passwords or private keys, in plaintext.
...
For example, SystemDiskCategory instead of SystemDisk.Category.
...
|InstanceType|String|Yes|Instance type. For more information about available value, see[Instance type families](../../../../intl.en-US/Product Introduction/Instance type families.md#), or query the latest instance type list by calling [DescribeInstanceTypes](intl.en-US/API Reference/Instances/DescribeInstanceTypes.md#).|
...
|
|Description|String|No|Description of an ECS instance.-   It can contain \[2, 256\] characters in length.
-   The description is displayed in the ECS console.
-   Default value: null.
-   Cannot begin with http:// or https://.

|
|InternetChargeType|String|No |Internet billing method. Optional value:-   PayByTraffic
...
|
|Password|String|No|Password of the ECS instance.-   It can be 8 to 30 characters in length and can contain the following types of characters: uppercase and lowercase letters, digits, and
...
|IoOptimized|String|No|Whether an ECS instance is I/O optimized or not. Optional values:-   none: Not I/O Optimized
-   optimized: I/O Optimized

For Generation I instances, the default value is none.For other instances, the default value is optimized.

|
|SystemDisk.Category|String|No|Cloud disk category of the system disk. Optional values:-   cloud: Basic cloud disk
-   cloud\_efficiency: Ultra cloud disk
-   cloud\_ssd: Cloud SSD
-   ephemeral\_ssd: Ephemeral SSD

For Generation I and non I/O Optimized instances, the default value is cloud.For other instances, the default value is cloud\_efficiency. Note: For I/O optimized instances, cloud\_ssd and cloud\_efficiency disks are supported. For non I/O Optimized instances, ephemeral\_ssd and cloud disks are supported.

|
|SystemDisk.Size|Integer|No|Size of the system disk, measured in GB. Value range: \[20, 500\]. The specified value must be equal to or greater than max\{20, ImageSize\}.  Default value: max\{40, ImageSize\}.|
...
Default value: false.|
|DataDisk.n.SnapshotId|String|No|Snapshot is used to create the data disk. After this parameter is specified, parameter `DataDisk.n.Size` is ignored, and the size of a new disk is the size of the specified snapshot. If the specified snapshot was created on or before July 15, 2013, this invocation is denied, and an error `InvalidSnapshot.TooOld` is returned.|
|DataDisk.n.DiskName|String|No|Name of a data disk.-   It can be \[2, 128\] characters in length. Must begin with an English letter or Chinese character. Can contain digits, colons\(:\), underscores \(\_\), or hyphens \(-\).
...
|
|DataDisk.n.DeleteWithInstance|Boolean|No|Whether a data disk is released along with the instance or not. Optional values:-   true: Indicates the disk is released with the instance.
...
|SpotStrategy|String|No|The spot strategy of a Pay-As-You-Go instance, and it takes effect only when parameter `InstanceChargeType` is PostPaid. Optional values:-   NoSpot: A regular Pay-As-You-Go instance
...
|Period|Integer|No|Unit: month. This parameter is valid when `InstanceChargeType` is `PrePaid`. Optional values:-   1-9
-   12
-   24
-   36
-   48
...
|
|PeriodUnit|String|No|Value:  week/month. When `PeriodUnit` is `Week`, -   Period is \{“1”, “2”, “3”, “4”\}
-   AutoRenewPeriod is \{“1”, “2”, “3”\}

When `PeriodUnit` is `Month`,-   Period is \{ “1”, “2”, “3”, “4”, “5”, “6”, “7”, “8”, “9”, “12”, “24”, “36”,”48”,”60”\}
-   AutoRenewPeriod is \{“1”, “2”, “3”, “6”, “12”\}

Default value: Month|
|AutoRenew|String|No|Whether to set AutoRenew. This parameter is valid when `InstanceChargeType` is `PrePaid`. Optional values:-   True: auto renewal.
-   False: not auto renewal.

Default value: False|
|AutoRenewPeriod|Integer|No|This parameter is required when  `AutoRenew` is  `True`. Optional values:-   1
-   2
-   3
-   6
-   12
...
|ClientToken|String|No|To guarantee the idempotence of the request. The value is generated by a client. It must be unique among all requests and can contain a maximum of 64 ASCII characters. For more information, see [How to ensure idempotence](intl.en-US/API Reference/Appendix/How to ensure idempotence.md#).|
|KeyPairName|String|No|Key pair name.-   This parameter is valid only for a Linux instance. For a Windows ECS instance, if a value is set for parameter KeyPairName, the password still takes effect.
...
|RamRoleName|String|No|Instance RAM role name.  The name is provided and maintained by *Resource Access Management \(RAM\)* and can be queried using [ListRoles](../../../../intl.en-US//Role Management Interface/ListRoles.md#). For more information, see RAM API [CreateRole](../../../../intl.en-US//Role Management Interface/CreateRole.md#) and [ListRoles](../../../../intl.en-US//Role Management Interface/ListRoles.md#).|
|SecurityEnhancementStrategy|String|No|Whether to enable security hardening. Optional values:-   Active: Enable security hardening. Works on system image only.
-   Deactive: Not enable security hardening. Works on all images.

|
|Tag.n.Key|String|No|The key of a tag,  of which n is from 1 to 5. It can be up to 64 characters in length and  cannot be an empty string. It cannot begin with aliyun,  http://, or https://.|
|Tag.n.Value|String|No|The value of a tag,  of which n is from 1 to 5. It can be up to 128 characters  in length and can be an empty string. Cannot begin with aliyun, http://, or https://.|
...
|InvalidVSwitchId.NotFound|The VSwitchId provided does not exist in our records.|403|The specified VSwitch does not exist.|
...
|InvalidKeyPairName.NotFound|The specified KeyPairName does not exist in our records.|404|The specified KeyPairName parameter does not exist.|
...
|PaymentMethodNotFound|No billing method has been registered on the account.|404|No billing method is specified for this account.|
