...
-   Security requirements for customized Linux images are as follows:
    ...
    -   When you log on to an instance for the first time through the [**Management Terminal**](intl.en-US/User Guide/Connect to instances/Connect to an instance by using the Management Terminal.md#) of the ECS console, you must change the initial default password if there is any. You can operate on the instance only after changing the password.
...
## Attentions on script parsing {#configureparsingscript .section}
