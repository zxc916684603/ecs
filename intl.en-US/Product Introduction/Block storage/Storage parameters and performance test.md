...
This article describes the performance indices of block storage, performance testing methods, and how to interpret the testing results.

## Performance indices of block storage {#section_b5z_vzv_ydb .section}

The main indices for measuring storage performance include IOPS, throughput, and latency.
...
|Parameter|SSD Cloud Disk|Ultra Cloud Disk|Basic Cloud Disk|
|:--------|:-------------|:---------------|:---------------|
|Capacity of a single disk|32,768 GiB|32,768 GiB|2,000 GiB|
|Max. IOPS|25,000\*|5,000|Several hundreds|
|Max. throughput|300 MBps\*|140 MBps|30 MBps−40 MBps|
|Formulas to calculate performance of a single disk\*\*|IOPS = min\{1800 + 30 \* capacity, 25000\}|IOPS = min\{1800 + 8 \* capacity, 5000\}|N/A|
|Throughput = min\{120 + 0.5 \* capacity, 300\} MBps|Throughput = min\{100 + 0.15 \* capacity, 140\} MBps|N/A|
|Data reliability|99.9999999%|99.9999999%|99.9999999%|
|API name|cloud\_ssd|cloud\_efficiency|cloud|
|Scenarios| -   Large or medium-sized relational databases, such as PostgreSQL, MySQL, Oracle, and SQL Server
...
|Data block size|Max. IOPS|Throughput|
|:--------------|:--------|:---------|
...
|
|-bs=4k| Indicates the size of each block for one I/O is 4 KB. If not specified, the default value 4 KB is used.

 When IOPS is tested, we recommend that you set `bs` to a small value, for example, 4k.

 When throughput is tested, we recommend that you set the `bs` to a large value, such as 1024k in this example command.
