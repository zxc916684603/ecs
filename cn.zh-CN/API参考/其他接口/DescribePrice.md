...
-   PayByTraffic：按带宽流量计费
 ...
 |
|InternetMaxBandwidthOut|Integer|否|5|公网出带宽最大值，单位为Mbps（Megabit per second）。取值范围：0~100

 默认值：0

 |
|IoOptimized|String|否|none|查询的实例是否为I/O优化实例。取值范围：
...
|404|InvalidSystemDisk.NotFound|The specified system disk does not exist.|系统盘不存在。|
