# Manage user data {#concept_pth_dbx_wgb .concept}
...
1.  Create a Linux instance by following the instructions in [Create an instance by using the wizard](reseller.en-US/Instances/Create an instance/Create an instance by using the wizard.md#).

    **Note:** During instance creation, enter user data in the **user data** box below **Advanced Options \(Instance RAM Roles and User Data cloud-init\)**. If your user data is Base64-encoded, select the **Enter Based64 Encoded Information** check box.

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/130417/156677788443603_en-US.png)

2.  After the instance starts, connect to it by following the instructions in [Overview of connecting to an ECS instance](reseller.en-US/Instances/Connect to instances/Overview.md#).
3.  Check the startup results against the configured user data. If exceptions occur, check the log files. The following output is from a CentOS instance which is configured with user data through an Upstart Job script:

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/130417/156677788543606_en-US.png)
...
1.  Find the target instance, and in the **Actions** column, choose **Instance Settings** \> **Set User Data**.
2.  In the displayed dialog box, enter the relevant information and click **OK**.

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/130417/156677788543608_en-US.png)
