...
-   您需要创建一台GPU计算型实例，确保实例能访问公网。具体操作，请参考[创建GPU计算型实例](cn.zh-CN/实例/选择实例规格/GPU计算型/创建GPU计算型实例.md#)。
...
3.  运行`reboot` 重启系统。
4.  再次[远程连接Linux实例](../../../../../cn.zh-CN/实例/连接实例/连接Linux实例/使用用户名密码验证连接Linux实例.md#)，并运行以下命令下载NVIDIA GRID驱动包。
...
6.  运行命令`nvidia-smi` 测试驱动安装结果。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/9838/155531814911965_zh-CN.png)
...
10. 配置X Server环境并重启。
    ...
    2.  编辑/etc/X11/xorg.conf，在`Section "Device"` 段添加GPU BusID，如本示例中为`BusID "PCI:0:7:0"`。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/9838/155531814911966_zh-CN.png)
...
2.  运行命令`startx` 启动X Server。
    ...
    **说明：** 
    ...
    -   `startx` 启动时可能会提示`hostname: Name or service not known`。这个提示不会影响X Server启动。您可以运行命令`hostname` 查得主机名后，再修改/etc/hosts文件，将`127.0.0.1` 后的`hostname` 改为本机的hostname。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/9838/155531814911967_zh-CN.png)
...
3.  开启一个新的SSH客户端终端，运行命令启动x11vnc。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/9838/155531815111968_zh-CN.png)
...
7.  按以下步骤使用`glxgears` 命令测试GRID驱动。
    ...
    2.  运行`glxgears` 启动齿轮图形测试程序。 如果出现如下图所示的窗口，表明GRID驱动正常工作。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/9838/155531815111970_zh-CN.png)
...
3.  运行`reboot` 重启系统。
4.  再次[远程连接Linux实例](../../../../../cn.zh-CN/实例/连接实例/连接Linux实例/使用用户名密码验证连接Linux实例.md#)，并运行以下命令下载并解压NVIDIA GRID驱动包。
...
8.  运行命令`nvidia-smi` 测试驱动是否安装成功。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/9838/155531815111971_zh-CN.png)
