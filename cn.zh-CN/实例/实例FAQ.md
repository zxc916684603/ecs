...
-   实例计费FAQ
    -   [按量付费转预付费时下单失败如何处理？](#section_bsy_tnq_wgb)
    -   [支付完成后多久能完成按量付费转预付费？](#section_lfq_vnq_wgb)
    -   [按量付费转预付费失败时如何处理？](#section_oml_wnq_wgb)
    -   [按量付费转预付费之后带宽计费方式是否发生变化？](#section_ph2_xnq_wgb)
    -   [我有一个按量付费转预付费订单，但是还没有支付，这时我升级了实例的配置，这个转换订单还有效吗？](#section_mbf_ynq_wgb)
    -   [预付费转按量付费失败时如何处理？](#section_fzl_5jr_l2b)
    -   [新创建的预付费实例在5天内转为按量付费，属于5天无理由退款吗？](#section_kw1_tqq_wgb)
-   预留实例券FAQ
...
-   管理实例FAQ
    -   [ECS Linux服务器到期续费后站点无法访问怎么办？](#section_zr1_wp4_08b)
    ...
    -   [购买ECS后如何备案域名？](#section_br4_f3k_1jw)
    ...
    -   [ECS实例中AliVulfix进程是什么？](#section_bnr_0sn_cww)
    -   [云服务器能防网络攻击吗？](#section_ctq_i74_gn9)
    -   [阿里云安全方面有哪些服务？](#section_14j_r01_ps1)
    -   [公网IP和私网IP是否独立，能指定或增加IP吗？](#section_dvt_enw_7gr)
    -   [云服务器能否做负载均衡？](#section_g6s_x91_9m2)
    -   [如何查询Linux实例的账号和密码？](#section_ka9_pvm_6fm)
    -   [如何检查Linux实例磁盘和进行分区格式化？](#section_cdh_blh_7dz)
    -   [如何向Linux实例上传文件？](#section_8mo_0zo_p27)
    -   [如何调整Linux实例目录文件的拥有者和拥有组？](#section_wet_cor_4rb)
    -   [如何更新Linux实例的软件源？](#section_jv7_tct_n90)
    -   [如何在Linux实例上搭建支持PHP的Web环境？](#section_as9_54l_1q9)
    -   [如何安装VNC Server以实现图形化访问Linux系统？](#section_apz_g8x_zdc)
    -   [为什么更改硬盘收费方式提示三次机会已用完？](#section_4jo_u47_eg2)
    -   [购买ECS实例后可以更换地域吗？](#section_wm7_rby_mij)
    -   [可以调整已购磁盘的分区大小吗？](#section_nih_446_koq)
    -   [如何将实例的固定公网IP更换为其它EIP？](#section_grq_3gj_n2r)
    -   [为什么无法将按量付费实例转为预付费实例？](#section_b1y_dqm_rzx)
-   升降配实例FAQ
    -   [我能升级包年包月ECS实例规格和配置吗？](#section_2ju_d8e_4af)
    -   [我能升级按量付费ECS实例规格和配置吗？](#section_k9f_0nr_x43)
    -   [升级ECS实例过程大约需要多久？](#section_0d7_eur_re8)
    -   [升级ECS实例产生的费用怎么计算？](#section_xp1_a5a_uzr)
    -   [升级ECS实例对我的云上业务的配置有什么影响吗？](#section_8hl_166_pwz)
    -   [如何升级ECS资源？](#section_3c2_9re_bpi)
    -   [为什么降配时实例预计退款为0？](#section_to5_4rg_1pp)
    -   [为什么升级实例配置后没有效果？](#section_ftn_d7x_vk5)
-   实例使用限制FAQ
...
如果依然没有资源，建议您耐心等待一段时间再购买。实例资源是动态的，如果资源不足，阿里云会尽快补充资源，但是需要一定时间。

您也可以使用到货通知功能：

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/130429/156086630648634_zh-CN.png)
...
## 支付完成后多久能完成按量付费转预付费？ {#section_lfq_vnq_wgb .section}

支付完成后，会有异步任务执行转换操作。目前同时转换1 ~ 20个实例的计费方式所需时间为1 ~ 4秒。转换完成后，您可以在控制台上看到实例的计费方式已经更新为**包年包月**。

## 按量付费转预付费失败时如何处理？ {#section_oml_wnq_wgb .section}

请[提交工单](https://selfservice.console.aliyun.com/ticket/createIndex?spm=a2c4g.11186623.2.7.8oI1Ug)。

## 按量付费转预付费之后带宽计费方式是否发生变化？ {#section_ph2_xnq_wgb .section}

不变。目前按量付费转包年包月功能只支持转换实例和云盘的计费方式，变更带宽计费方式请参见[变更公网带宽计费方式](cn.zh-CN/实例/升降配实例/升降配方式汇总.md#ChangeBilling)。

## 我有一个按量付费转预付费订单，但是还没有支付，这时我升级了实例的配置，转换订单还有效吗？ {#section_mbf_ynq_wgb .section}
...
## 预付费转按量付费失败时如何处理？ {#section_fzl_5jr_l2b .section}
...
如果出现以上明确的错误提示，建议您根据错误提示调整实例。如果问题仍旧存在，请[提交工单](https://selfservice.console.aliyun.com/ticket/createIndex?spm=a2c4g.11186623.2.7.8oI1Ug)。

## 新创建的预付费实例在5天内转为按量付费，属于5天无理由退款吗？ {#section_kw1_tqq_wgb .section}
...
如果您要连接的不是 I/O 优化实例，您需要在管理控制台上[重启实例](cn.zh-CN/实例/管理实例/重启实例.md#)，新密码才会生效。在ECS实例内部重启无效。
...
-   如果是 Windows 实例出现持续黑屏，您可以在界面左上角单击**发送远程命令** \> **CTRL+ALT+DELETE**，进入登录界面。
...
-   [通过浏览器在控制台上登录实例](cn.zh-CN/实例/连接实例/连接Linux实例/使用管理终端连接Linux实例.md#)
-   [PC端使用密码认证登录Linux实例](cn.zh-CN/实例/连接实例/连接Linux实例/使用用户名密码验证连接Linux实例.md#)
-   [PC端使用SSH密钥对登录Linux实例](cn.zh-CN/实例/连接实例/连接Linux实例/使用SSH密钥对连接Linux实例.md#)
-   [移动端动端登录Linux实例](cn.zh-CN/实例/连接实例/连接Linux实例/在移动设备上连接Linux实例.md#)

## ECS Linux服务器到期续费后站点无法访问怎么办？ {#section_zr1_wp4_08b .section}

问题描述：ECS到期后，服务器会处于停止状态。续费后启动服务器，发现站点无法访问。

问题分析：这种情况一般都是服务器启动后，服务器上的网站服务没有启动导致。

解决方法：

1.  远程登录到服务器上查看服务是否启动：

    ``` {#codeblock_75v_uvv_67u}
    #netstat -nltp //查看一下服务器的80端口是否监听状态
    ```

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/130429/156086630641861_zh-CN.png)

2.  如果未显示80端口信息，说明当前服务器站点服务没有启动，需手动开启：

    ``` {#codeblock_8an_f65_ov8}
    #/etc/init.d/httpd start          //适用于Apache环境
    #/etc/init.d/nginx start          // 适用于nginx环境
    ```

3.  Linux系统一般都是php+mysql网站环境，如果安装的是Apacehe环境，需启动MySQL；如果是Ngin环境，需启动 PHP和MySQL。PHP 和 MySQL常用的启动命令如下：

    ``` {#codeblock_4jb_ppd_prl}
    #/etc/init.d/php-fpm start        //启动PHP
    #/etc/init.d/mysqld start            //启动mysql
    ```

4.  执行完启动命令后，再用`netstat -nltp`查看80端口状态，重新访问站点验证问题是否解决。
...
-   如果AliyunService被禁用：将AliyunService修改为自动状态，然后重启云服务器 ECS。
-   如果AliyunService被删除：
    1.  执行以下命令重新添加服务：

        ``` {#codeblock_s90_l3e_h6d}
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/130429/156086630743179_zh-CN.png)
...
5.  在ECS实例上为www.example.com添加SSL证书和配置。

如问题还未解决，请[提交工单](https://selfservice.console.aliyun.com/ticket/createIndex)。

## 如何操作f1实例？ {#section_ug1_lb2_d7l .section}

f1实例试用申请获批之后，阿里云会向您共享一个FPGA开发镜像（目前只能支持CentOS 7u2）。您可以在镜像上免费试用Intel Quartus全套开发组件和f1相应器件的约束文件，体验一整套云上开发环境。
...
4.  关联成功后，继续调用API装载FPGA镜像。
    ...
    装载操作必须从f1实例所在的云服务器上发起，指定FPGA ID后，阿里云的底层服务会把对应的FPGA镜像烧制到实例对应的FPGA器件。
...
-   [使用f1 RTL（Register Transfer Level）](../cn.zh-CN/最佳实践/FaaS实例最佳实践/使用f1 RTL.md#)
...
1.  连接FTP服务。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/130429/156086630743224_zh-CN.png)

2.  进入正确的目录，Windows系统无需切换，Linux系统切换至htdocs。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/130429/156086630749119_zh-CN.png)

3.  运行put命令上传。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/130429/156086630849120_zh-CN.png)
...
4.  单击**连接**。安装后打开如图：
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/130429/156086630843228_zh-CN.png)

5.  在右侧选择要上传的目录（Windows不需要选择目录，Linux主机选择htdocs目录），然后在左边窗口选择要上传的文件，右击上传即可。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/130429/156086630843230_zh-CN.png)
    ...
    **说明：** 若安装Yummy FTP时系统提示：您的安全性偏好设置仅允许安装来自Mac App Store和被认可的开发者的应用程序，按以下步骤设置您的电脑。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/130429/156086630943238_zh-CN.png)


如问题还未解决，请[提交工单](https://selfservice.console.aliyun.com/ticket/createIndex)联系阿里云。

## 为什么无法在ECS实例上登录淘宝网？ {#section_n4z_oet_sdc .section}

问题描述：无法登录淘宝账号，登录后一直提示系统繁忙、提示输入验证码，验证码输入多次还是无法登录。

问题解决：淘宝网对外部访问的一些API限制导致。如果需要正常调用，建议通过使用聚石塔主机解决。更多详情请访问[聚石塔解决方案](https://cloud.tmall.com/)。

问题描述：ECS实例无法访问[手机淘宝](m.taobao.com)。

问题解决：移动淘宝主要供APP无线客户端访问，无线产品会有安全策略，阿里云无法保证ECS（属于PC端）一定可以访问成功。这是属于淘宝网对IP端相应的限制。
...
不支持。如果您需要兼顾灵活性和成本，可以在购买实例时选择按量付费实例，然后搭配预留实例券，更多信息请参见[预留实例券概述](cn.zh-CN/实例/选择实例购买方式/预留实例券/预留实例券概述.md#)。

## ECS实例如何查询云服务器IP地址归属地？ {#section_uvf_mwq_enl .section}

用户购买的ECS实例所在地域即是IP的归属地地域，可以通过[http://ip.taobao.com](http://ip.taobao.com)进行查询。

**说明：** 第三方网站可能会有IP地址数据库更新不及时的情况，会出现查询结果与实际地域不同的情况，请酌情选择。

## 云服务器默认提供数据库吗？ {#section_h97_kf4_xjp .section}

云服务器默认不提供数据库，您可以：

-   按照文档描述部署数据库，请参见[在ECS上部署数据库](../cn.zh-CN/建站教程/在ECS上部署数据库.md#)。
-   单独购买阿里云[RDS 数据库](https://rdsnext.console.aliyun.com/)服务。
-   使用镜像市场免费配置环境数据库。

## 云服务器上是否可以搭建数据库？ {#section_jli_u2y_g3m .section}

可以，您可以根据自己的需求进行软件安装和环境配置，ECS不限制安装。同时您也可以单独购买并使用阿里云[RDS 数据库](https://rdsnext.console.aliyun.com/)服务。

## 云服务器是否支持Oracle数据库？ {#section_j1b_acd_1f7 .section}

支持，建议您在安装前压测ECS性能是否可以满足您的数据库读写需求，您可以根据自己的需求安装软件并配置环境。

## ECS实例中AliVulfix进程是什么？ {#section_bnr_0sn_cww .section}

AliVulfix进程是云盾进行漏洞检测的程序，用于扫描云服务器是否有漏洞。

## 云服务器能防网络攻击吗？ {#section_ctq_i74_gn9 .section}

阿里云云服务器配有云盾服务，可以防御DDoS流量攻击。当出现网络攻击时，阿里云监控系统自动检测并清洗异常流量。如果攻击量非常大，阿里云会做黑洞处理，保护您的ECS实例安全。出于安全考虑，建议您自己安装防护软件，封闭不常用的端口。

## 阿里云安全方面有哪些服务？ {#section_14j_r01_ps1 .section}

阿里云提供云盾服务，结合阿里云云计算平台强大的数据分析能力。为您提供如安全漏洞检测、网页木马检测、面向云服务器用户提供的主机入侵检测和防御DDoS流量攻击等一站式安全服务。更多安全服务请查看[安全产品](https://security.aliyun.com/)页面。
...
## 如何查询Linux实例的账号和密码？ {#section_ka9_pvm_6fm .section}

云服务器ECS的账号和密码会在创建成功后发送到您账号绑定的手机和邮箱。Linux系统的默认管理员账号是root。

## 如何检查Linux实例磁盘和进行分区格式化？ {#section_cdh_blh_7dz .section}

目前磁盘空间和使用情况可以通过df –h命令来查询，查询磁盘信息可以通过fdisk –l命令来查询。 Linux实例上磁盘分区和格式化操作，请参见[格式化和挂载数据盘](../cn.zh-CN/个人版快速入门/步骤 4：格式化数据盘/Linux格式化数据盘.md#)。

## 如何向Linux实例上传文件？ {#section_8mo_0zo_p27 .section}

您可以使用FTP服务将文件上传到Linux实例。

## 如何调整Linux实例目录文件的拥有者和拥有组？ {#section_wet_cor_4rb .section}

在Web服务器里如果文件或者目录的权限不正确会导致访问网站出现403错误。所以首先要确认需要操作这些文件目录的进程是以什么身份运行的。在Linux里可以通过ps和grep查询。

另外可以使用ls –l来查询文件目录的拥有者和拥有组。使用chown命令修改权限：chown -R www.www /alidata/www/phpwind/可将目录/alidata/www/phpwind下的所有文件和目录的拥有者拥有组都修改为www账户。

## 如何更新Linux实例的软件源？ {#section_jv7_tct_n90 .section}

Linux系统下安装软件有时候需要使用新的软件源，帮助中心里的便捷工具即可完成Linux系统的源的更新，详细操作请参见：[云服务器 ECS Linux 软件源自动更新工具](https://help.aliyun.com/knowledge_detail/41177.html)。

## 如何在Linux实例上搭建支持PHP的Web环境？ {#section_as9_54l_1q9 .section}
...
## 如何安装VNC Server以实现图形化访问Linux系统？ {#section_apz_g8x_zdc .section}

请参见[云服务器 ECS Linux VNC Server 自动安装工具介绍](https://help.aliyun.com/knowledge_detail/41181.html)。

## 为什么更改硬盘收费方式提示三次机会已用完？ {#section_4jo_u47_eg2 .section}
...
出于系统安全和稳定性考虑，Windows和Linux实例均不支持系统盘二次分区。如果您强行使用第三方工具进行二次分区操作，可能引发未知风险，如系统崩溃、数据丢失等。数据盘可以进行二次分区，但容易导致数据丢失，不建议进行二次分区。
...
1.  先将固定带宽转换为按量带宽。
2.  [固定公网IP转换为EIP](../../../../../cn.zh-CN/用户指南/申请EIP/ECS固定公网IP转换为EIP.md#)。
3.  [解绑转换的得到的EIP](../../../../../cn.zh-CN/用户指南/解绑EIP.md#)。
4.  [绑定新的EIP](../../../../../cn.zh-CN/用户指南/绑定云资源/绑定ECS实例.md#)。

## 为什么无法将按量付费实例转为预付费实例？ {#section_b1y_dqm_rzx .section}
...
## 我能升级包年包月ECS实例规格和配置吗？ {#section_2ju_d8e_4af .section}

可以，请参见文档[预付费升级配置](cn.zh-CN/实例/升降配实例/升配预付费实例/预付费实例升级配置.md#)升级实例规格和带宽。

## 我能升级按量付费ECS实例规格和配置吗？ {#section_k9f_0nr_x43 .section}

可以，但需要预先停止实例。您可以参见文档[按量付费变更实例规格](cn.zh-CN/实例/升降配实例/升降配按量付费实例/按量付费实例变更实例规格.md#)或者通过API接口[ModifyInstanceSpec](../cn.zh-CN/API参考/实例/ModifyInstanceSpec.md#)升级按量付费实例的配置。

## 升级ECS实例过程大约需要多久？ {#section_0d7_eur_re8 .section}

-   升级包年包月实例规格无需预先停止实例，大约15分钟左右。
-   升级按量付费实例规格需要预先停止实例，大约15分钟左右。
-   升级实例带宽无需预先停止实例，大约5分钟左右。

## 升级ECS实例产生的费用怎么计算？ {#section_xp1_a5a_uzr .section}

升级ECS实例规格和配置时，页面上会显示费用信息。您也可以在[账户总览](https://expense.console.aliyun.com/)页面查询产生的费用详情。

## 升级ECS实例对我的云上业务的配置有什么影响吗？ {#section_8hl_166_pwz .section}

升级按量付费实例需要预先停止实例，升级包年包月实例也需要重启实例后生效配置，会使您的业务短暂中断，建议您在业务不繁忙时升级。升级后，实例能无缝对接业务，您无需重新配置云服务器ECS环境。

## 如何升级ECS资源？ {#section_3c2_9re_bpi .section}

有关升级云服务器ECS，请参见[升降配](cn.zh-CN/实例/升降配实例/升降配方式汇总.md#)。
...
## 为什么降配时实例预计退款为0？ {#section_to5_4rg_1pp .section}

降配时实例预计退款显示为0，可能是因为您在购买实例时享受了折扣价，降配后的配置按官网原价计算，差价可能会小于等于0元，这时均会显示为0。

## 为什么升级实例配置后没有效果？ {#section_ftn_d7x_vk5 .section}

升级实例配置后，需要在控制台[重启实例](cn.zh-CN/实例/管理实例/重启实例.md#)。

## ECS实例转移和更换公网IP地址有什么限制？ {#section_y49_iww_fvq .section}

转移和更换经典网络以及VPC网络的ECS实例公网IP地址的说明如下：

经典网络

-   不支持公网IP跨账号转移。
-   实例创建后6小时内，允许更换公网IP地址，最多可以更换3次。6小时后，不能再更换公网IP地址。具体操作，请参见[更换公网IP地址](../cn.zh-CN/网络/修改IPv4地址/修改公网IP地址.md#)。
-   使用高防IP业务后，最多支持更换10次源站IP。具体操作，请参见DDoS高防IP用户指南的[更换ECS IP地址](../../../../../cn.zh-CN/DDoS高防IP/用户指南/实例管理/更换 ECS IP.md#)。
...
-   如果实例已经分配了公网IP：
    -   实例创建后6小时内，允许更换公网IP地址，最多可以更换3次。具体操作，请参见[更换公网IP地址](../cn.zh-CN/网络/修改IPv4地址/修改公网IP地址.md#)。
    -   通过将公网IP转换为EIP的方式更换IP。具体操作，请参见[公网IP转换为弹性公网IP](../cn.zh-CN/网络/修改IPv4地址/公网IP转换为弹性公网IP.md#)。

如问题还未解决，请[提交工单](https://selfservice.console.aliyun.com/ticket/createIndex)。

## 云服务器ECS创建多少个网站？ {#section_999_02s_59y .section}

一个云服务器ECS最多支持绑定 5 个备案服务号（SN号），可供您完成5次网站备案。每个网站可以绑定多个域名。
